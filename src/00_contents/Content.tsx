...
const Content: React.FC<ContentProps> = ({ category }) => {
  ...
  useEffect(() => {
    ...
    // 이미 한 번 실행했다면 재실행 막기 (React 18 StrictMode 대비)
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (scrollTop + clientHeight >= scrollHeight - 10 && cards.length > 0) {
        // 현재 카드 배열을 한 번 더 붙여서 무한 스크롤 느낌만 제공
    ...
    <div className="content-wrap">
      ...
      <div className="infinite-content">
        ...
        <div className="infinite-wrap">
          ...
          >
            {/* 여기서는 cards만 렌더 (이미 state에서 배열 복제해줌) */}
            {cards.map((card, i) => (
